...
from pretrained_paths import SDXL_TURBO_DIR, IP_ADAPTER_DIR
...
class Generator4Embeds:
    def __init__(self, num_inference_steps=1, device='cuda',
                 sdxl_model_path=None, ip_adapter_path=None) -> None:
    ...
    def generate_batch(self, image_embeds_batch, text_prompts=None, generator=None):
        """Generate images from a batch of embeddings in a single forward pass.

        Args:
            image_embeds_batch: (B, embed_dim) tensor
            text_prompts: list of B strings, or None for empty prompts
            generator: optional torch.Generator
        Returns:
            list of B PIL images
        """
        B = image_embeds_batch.shape[0]
        image_embeds_batch = image_embeds_batch.to(device=self.device, dtype=self.dtype)
        if text_prompts is None:
            text_prompts = [''] * B

        result = self.pipe.generate_ip_adapter_embeds(
            prompt=text_prompts,
            ip_adapter_embeds=image_embeds_batch,
            num_inference_steps=self.num_inference_steps,
            guidance_scale=0.0,
            generator=generator,
        )
