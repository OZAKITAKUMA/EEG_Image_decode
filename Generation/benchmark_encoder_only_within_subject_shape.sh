...
OUTPUT_DIR="${OUTPUT_DIR:-./outputs/benchmark_encoder_only/trash}"
...
for SUBJECT in ${SUBJECTS}; do
    ...
    case "${SUBJECT}" in
        sub-01) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-01/08-15_18-11/best.pth" ;;
        sub-02) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-02/08-15_18-18/best.pth" ;;
        sub-03) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-03/08-15_18-26/best.pth" ;;
        sub-04) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-04/08-15_18-33/best.pth" ;;
        sub-05) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-05/08-15_18-41/best.pth" ;;
        sub-06) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-06/08-15_21-48/best.pth" ;;
        sub-07) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-07/08-15_21-55/best.pth" ;;
        sub-08) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-08/08-17_06-07/best.pth" ;;
        sub-09) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-09/08-17_06-14/best.pth" ;;
        sub-10) ENCODER_PATH="${MODEL_SAVE_DIR}/encoder/sub-10/08-17_06-22/best.pth" ;;
        *)
    ...
    if [ ! -f "${ENCODER_PATH}" ]; then
        echo "[ERROR] Encoder checkpoint not found for ${SUBJECT}: ${ENCODER_PATH}"
        continue
    fi

    echo "  Encoder path: ${ENCODER_PATH}"
    EVAL_OUTPUT_DIR="${OUTPUT_DIR}/${SUBJECT}"
